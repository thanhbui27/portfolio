import { Code2, Cpu, ExternalLink, Play, Star, TerminalSquare } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import type { Project, ProjectDetailFeature, SiteConfig } from "@/types/portfolio";
...
const sorted = (items: ProjectDetailFeature[]) => [...items].sort((a, b) => a.order - b.order);
...
function DetailButton({
  ...
  return (
    ...
    >
      {tone === "white" ? <Code2 aria-hidden="true" className="h-5 w-5" strokeWidth={3} /> : <Play aria-hidden="true" className="h-5 w-5" fill="currentColor" strokeWidth={3} />}
      {children}
    </Link>
  );
}
  ...
  return (
    <div className="space-y-3">
      {sorted(items).map((item) => (
        <div className="relative border-4 border-ink bg-[#f9f9f9] px-4 py-3 pl-10" key={item.title}>
          <Star
            aria-hidden="true"
          ...
          <p className="text-sm leading-relaxed sm:text-base">
            <span className="font-display text-lg font-bold uppercase leading-none">{item.title}: </span>
            {item.description}
          </p>
        </div>
      ))}
  ...
  return (
    <div className="flex flex-wrap gap-3 bg-[#e9ddff] p-4">
      {sorted(items).map((item, index) => (
        <span className="border-4 border-ink bg-[#f9f9f9] px-3 py-2 font-mono text-xs font-bold" key={item.title}>
          <span
            aria-hidden="true"
...
export function GameProjectDetail({ project, labels }: ProjectDetailLayoutProps) {
  const demoUrl = project.unityWebglUrl ?? project.liveDemoUrl;

  return (
    <main className="bg-[linear-gradient(90deg,rgba(180,31,83,.12)_0_64px,transparent_64px),linear-gradient(0deg,rgba(180,31,83,.12)_5%,transparent_5%),linear-gradient(90deg,rgba(180,31,83,.12)_5%,transparent_5%)] bg-[length:100%_100%,32px_32px,32px_32px] px-4 py-14 sm:px-6 lg:py-20">
      <section className="mx-auto max-w-6xl text-center">
        <div className="inline-block -rotate-2 border-4 border-ink bg-banana px-5 py-3 shadow-retro-sm">
          <p className="font-mono text-xs font-bold uppercase">{project.detail.levelLabel}</p>
        </div>
      ...
      <section className="mx-auto mt-12 max-w-5xl border-4 border-ink bg-[#f9f9f9] p-4 shadow-retro">
        <div className="relative aspect-[16/9] overflow-hidden border-4 border-ink bg-ink">
          <Image alt={project.title} className="object-cover" fill sizes="100vw" src={project.images[0]} unoptimized />
      ...
      <section className="mx-auto mt-14 grid max-w-6xl gap-8 lg:grid-cols-[360px_1fr]">
        <aside className="space-y-8">
          <div className="rotate-[-1deg] border-4 border-ink bg-grape p-1 shadow-retro">
            <div className="flex items-center justify-between bg-ink px-4 py-3 text-[#f9f9f9]">
              <h2 className="break-words font-display text-xl font-bold uppercase leading-none sm:text-3xl">{project.detail.specsTitle}</h2>
        ...
        <div className="space-y-8">
          <article className="border-4 border-ink bg-[#f9f9f9] shadow-retro">
            <div className="bg-ink px-5 py-3 text-[#f9f9f9]">
              <h2 className="break-words font-display text-xl font-bold uppercase leading-none sm:text-3xl">{project.detail.missionTitle}</h2>
            </div>
            <div className="space-y-4 p-5 text-lg leading-relaxed">
              ...
              <div className="border-4 border-ink bg-[#1b1b1b] p-4 font-mono text-sm font-bold text-mint">
                {project.detail.missionLog.map((line) => (
                  <p key={line}>
                    <TerminalSquare aria-hidden="true" className="mr-2 inline h-4 w-4" strokeWidth={3} />
                    {line}
                  </p>
                ))}
              </div>
          ...
          <article className="border-4 border-ink bg-[#e2e2e2] p-1 shadow-retro">
            <div className="bg-ink px-5 py-3 text-[#f9f9f9]">
              <h2 className="break-words font-display text-xl font-bold uppercase leading-none sm:text-3xl">{project.detail.featuresTitle}</h2>
            </div>
            <div className="grid gap-4 p-5 md:grid-cols-3">
              {sorted(project.detail.coreFeatures).map((feature) => (
                <div className="border-4 border-ink bg-[#f9f9f9] p-4" key={feature.title}>
                  <h3 className="font-display text-xl font-bold uppercase leading-none">{feature.title}</h3>
                  <p className="mt-3 text-sm leading-relaxed">{feature.description}</p>
                </div>
              ))}
            </div>
          </article>
          ...
          <div className="flex flex-wrap gap-4">
            <DetailButton href={demoUrl} tone="pink">{project.archive.buttonLabel}</DetailButton>
            <DetailButton href={project.githubUrl} tone="white">{labels.repository}</DetailButton>
          </div>
        </div>
      </section>
...
export function WebProjectDetail({ project, labels }: ProjectDetailLayoutProps) {
  return (
    <main className="bg-[linear-gradient(90deg,rgba(180,31,83,.12)_0_64px,transparent_64px),linear-gradient(0deg,rgba(180,31,83,.12)_5%,transparent_5%),linear-gradient(90deg,rgba(180,31,83,.12)_5%,transparent_5%)] bg-[length:100%_100%,32px_32px,32px_32px] px-4 py-14 sm:px-6 lg:py-20">
      <section className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1fr_.95fr] lg:items-center">
        <div>
          <div className="inline-block -rotate-2 border-4 border-ink bg-banana px-5 py-3 shadow-retro-sm">
            <p className="font-mono text-xs font-bold uppercase">{project.detail.levelLabel}</p>
          </div>
          ...
          <div className="mt-8 flex flex-wrap gap-4">
            <DetailButton href={project.liveDemoUrl} tone="pink">{project.archive.buttonLabel}</DetailButton>
            <DetailButton href={project.githubUrl} tone="white">{labels.repository}</DetailButton>
          </div>
        </div>
        ...
        <div className="rotate-1 border-4 border-ink bg-[#e2e2e2] p-5 shadow-retro">
          <div className="relative aspect-[16/9] overflow-hidden border-4 border-ink bg-[#f9f9f9]">
            <Image alt={project.title} className="object-cover" fill sizes="(min-width: 1024px) 50vw, 100vw" src={project.images[0]} unoptimized />
            {project.detail.stickerLabel ? (
      ...
      <section className="mx-auto mt-14 grid max-w-6xl gap-8 lg:grid-cols-[1fr_360px]">
        <div className="space-y-8">
          <article className="relative border-4 border-ink bg-[#f9f9f9] shadow-retro">
            <div className="bg-ink px-5 py-3 text-[#f9f9f9]">
              <h2 className="break-words font-display text-xl font-bold uppercase leading-none sm:text-3xl">{project.detail.overviewTitle}</h2>
            </div>
          ...
          <article className="border-4 border-ink bg-[#e2e2e2] p-1 shadow-retro">
            <div className="bg-ink px-5 py-3 text-[#f9f9f9]">
              <h2 className="break-words font-display text-xl font-bold uppercase leading-none sm:text-3xl">{project.detail.loadoutTitle}</h2>
            </div>
            <div className="p-5">
        ...
        <aside className="space-y-8">
          <div className="rotate-1 border-4 border-ink bg-grape p-1 shadow-retro">
            <div className="flex items-center justify-between bg-ink px-4 py-3 text-[#f9f9f9]">
              <h2 className="break-words font-display text-xl font-bold uppercase leading-none sm:text-3xl">{project.detail.specsTitle}</h2>
              <ExternalLink aria-hidden="true" className="h-5 w-5" strokeWidth={3} />
            </div>
            <TechPills items={project.detail.specs} />
          </div>
