...
export function SkillsSection({ skills, content }: SkillsSectionProps) {
  ...
  return (
    <MotionSection className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:py-16" id="skills">
      ...
      <div className="mt-12 grid gap-7 md:grid-cols-2 xl:grid-cols-3">
        {skillsByCategory.map((group) => (
          <article
            className={`w-full max-w-[calc(100vw-2rem)] border-4 border-ink p-4 shadow-retro sm:max-w-none ${
            ...
            <div className="space-y-5 bg-[#f9f9f9] p-5">
              ...
              <div className={`grid gap-3 ${group.span === "wide" ? "sm:grid-cols-2" : ""}`}>
                {group.skills.map((skill) => (
                  <div
                    className="flex flex-col items-start justify-between gap-2 border-4 border-ink bg-white px-3 py-2 sm:flex-row sm:items-center sm:gap-3"
                    key={skill.id}
                  >
